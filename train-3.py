...
if __name__ == "__main__":
    ...
    if not os.path.exists(f'./checkpoints/deepmsn_{date_time}'):
        os.makedirs(f'./checkpoints/deepmsn_{date_time}')
    ...
    for t in range(epochs):
        ...
        with torch.no_grad():
            for batch in test_dataloader:
                ...
                # Replace the incorrect accuracy calculation:
                # correct += (prob.argmax(1) == y.argmax(1)).type(torch.float).sum().item()
                
                # With proper multi-label accuracy:
                # Option A: Exact match accuracy (all labels must match exactly)
                pred_binary = (prob > 0.5).float()
                exact_match = ((pred_binary == y).sum(dim=1) == y.shape[1]).float().sum().item()
                correct += exact_match
                
                # Option B: Hamming accuracy (average per-label accuracy)
                # hamming_acc = ((pred_binary == y).sum().item()) / (y.shape[0] * y.shape[1])
                # correct += hamming_acc * y.shape[0]  # Scale back to sample count
        ...
        correct /= size
        # print(f"Test Error: \n Accuracy: {(100*correct):>0.2f}%, Best F1: {best_f1:.4f} @ {best_threshold}, Avg loss: {test_loss:>8f} \n")
        print(f"Test Error: \n Accuracy: {(100*correct):>0.2f}%, Avg Test loss: {test_loss:>8f} \n")
        
        # Save model checkpoint every 100 epochs
        if (t + 1) % 100 == 0:
            save_path = f'./checkpoints/deepmsn_{date_time}/model_epoch_{t+1}.pth'
