...
if __name__ == "__main__":
    ...
    print(f"Creating checkpoints directory: ./checkpoints/dm_ctdnsmpl_2k_{date_time}")
    if not os.path.exists(f'./checkpoints/dm_ctdnsmpl_2k_{date_time}'):
        os.makedirs(f'./checkpoints/dm_ctdnsmpl_2k_{date_time}')
    ...
    with open(f'./checkpoints/dm_ctdnsmpl_2k_{date_time}/train_indices.txt', 'w') as f:
        f.write('\n'.join(map(str, train_indices)))
    
    val_indices = []
    for batch in val_dataloader:
        val_indices.extend(batch['index'].tolist())
    with open(f'./checkpoints/dm_ctdnsmpl_2k_{date_time}/val_indices.txt', 'w') as f:
        f.write('\n'.join(map(str, val_indices)))
    
    test_indices = []
    for batch in test_dataloader:
        test_indices.extend(batch['index'].tolist())
    with open(f'./checkpoints/dm_ctdnsmpl_2k_{date_time}/test_indices.txt', 'w') as f:
    ...
    for t in range(epochs):
        ...
        import csv
        csv_file = f'./checkpoints/dm_ctdnsmpl_2k_{date_time}/training_log.csv'
        ...
        if (t + 1) % 100 == 0:
            save_path = f'./checkpoints/dm_ctdnsmpl_2k_{date_time}/model_epoch_{t+1}.pth'
    ...
    with open(f'./checkpoints/dm_ctdnsmpl_2k_{date_time}/final_stats.json', 'w') as f:
        json.dump(final_stats, f, indent=2)
    
    # Save final model
    final_save_path = f'./checkpoints/dm_ctdnsmpl_2k_{date_time}/final_model.pth'
