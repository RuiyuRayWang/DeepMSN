...
if __name__ == "__main__":
    ...
    if config['train']['resume_from_checkpoint']:
        ...
        # Check consistency of augmentation settings
        if 'augmentation_used' in checkpoint:
            if checkpoint['augmentation_used']:
                print("✓ Checkpoint used augmentation - recreating augmented dataset")
                # Verify augmentation settings match
                saved_aug_kwargs = checkpoint.get('augment_kwargs', {})
                current_aug_kwargs = config.get('augment_kwargs', {})
                if saved_aug_kwargs != current_aug_kwargs:
                    print("⚠️ Warning: Augmentation settings differ from checkpoint")
                    print(f"  Saved: {saved_aug_kwargs}")
                    print(f"  Current: {current_aug_kwargs}")
            else:
                print("⚠️ Checkpoint did not use augmentation - consider consistency")
        ...
        # ⚠️ CRITICAL FIX: Create both base and augmented datasets
        base_dataset = TopicDataset(config=config, augment=False)
        augmented_dataset = TopicDataset(config=config, augment=True)
        
        print("Augmentation Statistics (resumed):")
        stats = augmented_dataset.get_augmentation_stats()
        for key, value in stats.items():
            print(f"  {key}: {value}")
        
        # Load the saved indices
        with open(f'{checkpoint_path}/train_indices.txt', 'r') as f:
            train_original_indices = [int(line.strip()) for line in f.readlines()]
        ...
        # ⚠️ CRITICAL FIX: Recreate augmented training indices
        # The train_indices.txt contains ORIGINAL indices, need to map to augmented indices
        train_aug_indices = []
        for aug_idx, (_, _, orig_idx, _) in enumerate(augmented_dataset.augmented_regions):
            if orig_idx in train_original_indices:
                train_aug_indices.append(aug_idx)
        
        # Create subsets with proper augmentation
        train_subset = torch.utils.data.Subset(augmented_dataset, train_aug_indices)  # Augmented
        val_subset = torch.utils.data.Subset(base_dataset, val_indices)              # Non-augmented
        test_subset = torch.utils.data.Subset(base_dataset, test_indices)            # Non-augmented
        
        print(f"Resumed dataset sizes:")
        print(f"  Augmented training: {len(train_subset)}")
        print(f"  Validation: {len(val_subset)}")
        print(f"  Test: {len(test_subset)}")
        
        # Define data loaders for training and testing
        train_dataloader = DataLoader(train_subset, batch_size=batch_size, shuffle=True, num_workers=1)
        val_dataloader = DataLoader(val_subset, batch_size=batch_size, shuffle=False, num_workers=1)
        test_dataloader = DataLoader(test_subset, batch_size=batch_size, shuffle=False, num_workers=1)
        

    else:
        # Initialize checkpoints directory
        date_time = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
        checkpoint_path = f'checkpoints/dm_ctdnsmpl_3k_augment_{date_time}'
        ...
        # Split the base dataset indices for train/val/test
        train_size = int(0.7 * len(base_dataset))
        val_size = int(0.15 * len(base_dataset))
        test_size = len(base_dataset) - val_size - train_size
        
        train_indices, val_indices, test_indices = torch.utils.data.random_split(
            range(len(base_dataset)), [train_size, val_size, test_size]
        )
        
        # For training: filter augmented dataset to only include training indices
        train_aug_indices = []
        for aug_idx, (_, _, orig_idx, _) in enumerate(augmented_dataset.augmented_regions):
            if orig_idx in train_indices.indices:
                train_aug_indices.append(aug_idx)
        
        # Create subsets
        train_subset = torch.utils.data.Subset(augmented_dataset, train_aug_indices)
        ...
        print(f"  Original dataset: {len(base_dataset)}")
        print(f"  Augmented training: {len(train_subset)}")
        print(f"  Validation: {len(val_subset)}")
        print(f"  Test: {len(test_subset)}")
        
        # Save indices for reproducibility
        train_original_indices = [augmented_dataset.augmented_regions[i][2] for i in train_aug_indices]
        
        with open(f'{checkpoint_path}/train_indices.txt', 'w') as f:
            f.write('\n'.join(map(str, train_original_indices)))
        with open(f'{checkpoint_path}/val_indices.txt', 'w') as f:
            f.write('\n'.join(map(str, val_indices.indices)))
        with open(f'{checkpoint_path}/test_indices.txt', 'w') as f:
            f.write('\n'.join(map(str, test_indices.indices)))
    ...
    for t in range(epoch_start - 1, epochs):
        ...
        if (t + 1) % save_every_n_epochs == 0:
            checkpoint = {
                ...
                'augmentation_used': True,  # Track if augmentation was used
                'augment_kwargs': config.get('augment_kwargs', {}),  # Save augmentation settings
                'epoch': t + 1,
                'train_loss': avg_train_loss,
                'val_loss': val_loss
            }
